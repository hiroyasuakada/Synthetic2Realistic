...
class TNetRealModel(BaseModel):
    ...
    def initialize(self, opt, labeled_dataset=None, unlabeled_dataset=None):
        ...
        self.loss_names = ['lab_t', 'lab_smooth']
        self.visual_names = ['img_t', 'lab_t', 'lab_t_g']
    ...
    def backward_task(self):
        ...
        self.output_t_g = self.net_img2task(self.img_t)

        size = len(self.output_t_g)
        self.lab_t_g = self.output_t_g[1:] 

        lab_real = task.scale_pyramid(self.lab_t, size-1)
        task_loss = 0
        ...
        total_loss = self.loss_lab_t + self.loss_lab_smooth
