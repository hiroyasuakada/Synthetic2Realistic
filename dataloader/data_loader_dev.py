...
from util.convert_exr2array import exr2array
...
def dataloader(opt, gp=False):
    if not gp:
        ...
        dataset = torch.utils.data.DataLoader(
            datasets, batch_size=opt.batch_size, shuffle=opt.shuffle, num_workers=int(opt.num_threads)
        )
        return dataset
    else:
...
class CreateFullDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if img_source_path.endswith('.exr'):
            img_source = self.load_img(img_source_path, exr=True, gray=False)
        else: 
            img_source = self.load_img(img_source_path, exr=False, gray=False)

        if img_target_path.endswith('.exr'):
            img_target = self.load_img(img_target_path, exr=True, gray=False)
        else: 
            img_target = self.load_img(img_target_path, exr=False, gray=False)

        img_source = torch.from_numpy(img_source.astype(np.float)).clone().permute(2, 0, 1)
        img_target = torch.from_numpy(img_target.astype(np.float)).clone().permute(2, 0, 1)
        ...
        if self.opt.isTrain:
            ...
            if lab_source_path.endswith('.exr'):
                lab_source = self.load_img(lab_source_path, exr=True, gray=True)
            else: 
                lab_source = self.load_img(lab_source_path, exr=False, gray=True)

            if lab_target_path.endswith('.exr'):
                lab_target = self.load_img(lab_target_path, exr=True, gray=True)
            else: 
                lab_target = self.load_img(lab_target_path, exr=False, gray=True)

            lab_source = torch.from_numpy(lab_source.astype(np.float)).clone().unsqueeze(0)
            lab_target = torch.from_numpy(lab_target.astype(np.float)).clone().unsqueeze(0)
            ...
            return {'img_source': img_source.float(), 'img_target': img_target.float(),
                    'lab_source': lab_source.float(), 'lab_target': lab_target.float(),
            ...
            lab_source = self.transform_no_augment_depth(lab_source).permute(2, 0, 1)
            lab_target = self.transform_no_augment_depth(lab_target).permute(2, 0, 1)
    ...
    def load_img(self, path, exr=False, gray=False):
        if exr:
            img = exr2array(path, gray=gray)
            img = cv2.resize(img, (self.opt.load_size[0], self.opt.load_size[1]), interpolation=cv2.INTER_CUBIC)
        else:
            if gray:
                img = Image.open(path)
            else:
                img = Image.open(path).convert('RGB')
            img = img.resize([self.opt.load_size[0], self.opt.load_size[1]], Image.BICUBIC)
            img = np.array(img, np.float) / 255.0
...
class CreateLabeledDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        return {'img_source': img_source.float(), 'lab_source': lab_source.float(), 
...
class CreateUnlabeledDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        return {'img_target': img_target.float(), 'lab_target': lab_target.float(),
